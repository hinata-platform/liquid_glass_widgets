...
the morph, with no perceptible change to the final look. The
`notification-popover-performance` writeup in the Hinata app repo has the
before/after frame-timing captures this was derived from.
...
Restores the pre-`0.21.6+hinata.4` behaviour:
